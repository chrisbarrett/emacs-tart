...
let test_search_path_of_dirs () =
  with_temp_dirs 2 (fun dirs ->
    ...
    | [dir1; dir2] ->
        ...
        write_file (Filename.concat dir2 "mymod.tart") {|
        ...
        Alcotest.(check (option string)) "finds file in second dir" expected result
...
let test_search_path_with_stdlib () =
  with_temp_dirs 2 (fun dirs ->
    ...
    | [search_dir; stdlib_dir] ->
        ...
        write_file (Filename.concat stdlib_dir "cl-lib.tart") {|
...
let test_search_path_prefers_search_over_stdlib () =
  with_temp_dirs 2 (fun dirs ->
    ...
    | [search_dir; stdlib_dir] ->
        ...
        Alcotest.(check (option string)) "search dir wins over stdlib" expected result
    | _ -> Alcotest.fail "expected 2 dirs")
...
let test_find_sibling () =
  with_temp_dirs 1 (fun dirs ->
    ...
    | [dir] ->
        ...
        Alcotest.(check (option string)) "finds sibling" (Some tart_path) result
...
(** Test that sibling files take precedence over search path.
    R16: "The first match wins, allowing project-local overrides." *)
...
(** Test full discovery order: sibling -> search path -> stdlib.
...
let test_full_discovery_order () =
  with_temp_dirs 3 (fun dirs ->
    ...
    | [project_dir; search_dir; stdlib_dir] ->
        ...
        write_file (Filename.concat stdlib_dir "stdlib-mod.tart") {|
        ...
        (match result with
         ...
         | Some sig_file ->
             let has_stdlib_var = List.exists
                 (function
  ...
  with_temp_dirs 1 (fun dirs ->
    ...
    | [dir] ->
        ...
        (match result with
         ...
         | Some sig_file ->
             Alcotest.(check string) "module name from filename"
               "example" sig_file.sig_module;
             Alcotest.(check int) "has 2 declarations" 2 (List.length sig_file.sig_decls))
    | _ -> Alcotest.fail "expected 1 dir")
...
let test_parse_invalid_file () =
  with_temp_dirs 1 (fun dirs ->
    ...
    | [dir] ->
        ...
        write_file tart_path "(defun bad syntax";  (* Invalid syntax *)
        let result = Search_path.parse_signature_file tart_path in
        Alcotest.(check bool) "invalid file returns None" true (Option.is_none result)
    | _ -> Alcotest.fail "expected 1 dir")
...
(** Test that loaded signatures from search path work with type checker.
    R15: "Call to seq-map with wrong types produces error" *)
let test_loaded_signature_type_checks () =
  with_temp_dirs 1 (fun dirs ->
    match dirs with
    | [search_dir] ->
        (* Create a signature file in search path *)
        write_file (Filename.concat search_dir "math.tart") {|
          (defun math-add (int int) -> int)
          (defun math-concat (string string) -> string)
        |};
        let sp = Search_path.of_dirs [search_dir] in
        let env =
          match Search_path.load_module ~search_path:sp ~env:Type_env.empty "math" with
        ...
        Alcotest.(check int) "no errors for correct call" 0 (List.length errors1);
        ...
        Alcotest.(check bool) "error for wrong types" true (List.length errors2 > 0)
    | _ -> Alcotest.fail "expected 1 dir")

(** Test that search path resolver works with open/include.
    This verifies the resolver integrates correctly with sig_loader. *)
let test_resolver_with_open () =
  with_temp_dirs 1 (fun dirs ->
    match dirs with
    | [search_dir] ->
        (* Create a base module with a type alias *)
        write_file (Filename.concat search_dir "base-types.tart") {|
          (type int-list (list int))
        |};
        (* Create a module that opens base-types *)
        write_file (Filename.concat search_dir "consumer.tart") {|
          (open 'base-types)
          (defun process (int-list) -> int)
        |};
        let sp = Search_path.of_dirs [search_dir] in
        let env =
          match Search_path.load_module ~search_path:sp ~env:Type_env.empty "consumer" with
        ...
        (match Type_env.lookup "process" env with
         ...
         | Some scheme ->
             ...
             Alcotest.(check bool) "type alias expanded"
...
let test_prepend_append_dir () =
  with_temp_dirs 3 (fun dirs ->
    ...
    | [dir1; dir2; dir3] ->
        ...
        Alcotest.(check (option string)) "prepended dir searched"
          (Some (Filename.concat dir1 "mod1.tart")) result1;
        (* mod3 should be found in dir3 (appended) *)
        let result3 = Search_path.find_signature sp "mod3" in
        Alcotest.(check (option string)) "appended dir searched"
    ...
    [
      ( "search-path-config",
        [
          ...
          Alcotest.test_case "search path of dirs" `Quick test_search_path_of_dirs;
          Alcotest.test_case "search path precedence" `Quick test_search_path_precedence;
          Alcotest.test_case "search path with stdlib" `Quick test_search_path_with_stdlib;
          Alcotest.test_case "prefers search over stdlib" `Quick test_search_path_prefers_search_over_stdlib;
          Alcotest.test_case "prepend and append dir" `Quick test_prepend_append_dir;
        ] );
      ( "sibling-files",
        [
          Alcotest.test_case "find sibling" `Quick test_find_sibling;
          Alcotest.test_case "sibling not found" `Quick test_find_sibling_not_found;
        ] );
      ( "discovery-order",
        [
          Alcotest.test_case "sibling takes precedence" `Quick test_sibling_takes_precedence;
          Alcotest.test_case "full discovery order" `Quick test_full_discovery_order;
        ] );
      ( "parsing",
        [
          Alcotest.test_case "parse signature file" `Quick test_parse_signature_file;
