...
let test_generalize_tvar_at_higher_level () =
  ...
  setup ();
  let tv = fresh_tvar 1 in  (* level 1 *)
  let scheme = G.generalize 0 tv in  (* outer level 0 *)
  match scheme with
  | Env.Poly ([_], _) -> ()
...
let test_no_generalize_tvar_at_same_level () =
  ...
  setup ();
  let tv = fresh_tvar 0 in  (* level 0 *)
  let scheme = G.generalize 0 tv in  (* outer level 0 *)
  match scheme with
  | Env.Mono _ -> ()
...
let test_let_identity_polymorphic () =
  ...
  let env = Env.extend_mono "progn"
    (arrow [Prim.any; Prim.any] Prim.any) Env.empty in
  let _ = infer_and_solve ~env
    "(let ((id (lambda (x) x)))
       (progn (id 1) (id \"s\")))" in
    ...
    [
      ( "syntactic values",
        [
          ...
          Alcotest.test_case "application not value" `Quick test_application_not_value;
          Alcotest.test_case "if not value" `Quick test_if_not_value;
          Alcotest.test_case "let not value" `Quick test_let_not_value;
        ] );
      ( "generalize",
        [
          Alcotest.test_case "tvar at higher level" `Quick test_generalize_tvar_at_higher_level;
          Alcotest.test_case "no generalize same level" `Quick test_no_generalize_tvar_at_same_level;
          Alcotest.test_case "arrow with tvars" `Quick test_generalize_arrow_with_tvars;
          Alcotest.test_case "nested tvars" `Quick test_generalize_nested_tvars;
        ] );
      ( "let polymorphism",
        [
          Alcotest.test_case "identity polymorphic" `Quick test_let_identity_polymorphic;
          Alcotest.test_case "identity at int" `Quick test_let_identity_used_at_int;
          Alcotest.test_case "identity at string" `Quick test_let_identity_used_at_string;
          Alcotest.test_case "const polymorphic" `Quick test_let_const_polymorphic;
          Alcotest.test_case "let* generalization" `Quick test_let_star_generalization;
        ] );
      ( "value restriction",
        [
