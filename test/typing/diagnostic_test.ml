...
let test_type_mismatch_diagnostic () =
  ...
  let d = Diag.type_mismatch ~span ~expected:Types.Prim.int ~actual:Types.Prim.string () in
  ...
  Alcotest.(check string) "expected is Int" "Int" (Types.to_string (Option.get d.expected));
  Alcotest.(check string) "actual is String" "String" (Types.to_string (Option.get d.actual))

let test_arity_mismatch_diagnostic () =
  let span = Loc.dummy_span in
  let d = Diag.arity_mismatch ~span ~expected:2 ~actual:3 () in
  Alcotest.(check bool) "is error" true (Diag.is_error d);
  Alcotest.(check bool) "message contains arity info" true
    (String.length d.message > 0);
  Alcotest.(check bool) "message mentions 2" true
    (contains_pattern (Str.regexp "2") d.message);
  Alcotest.(check bool) "message mentions 3" true
    (contains_pattern (Str.regexp "3") d.message)

let test_occurs_check_diagnostic () =
  let span = Loc.dummy_span in
  let d = Diag.occurs_check ~span ~tvar_id:42 ~typ:(Types.list_of Types.Prim.int) () in
  Alcotest.(check bool) "is error" true (Diag.is_error d);
  Alcotest.(check bool) "message mentions infinite" true
...
let test_format_span_multiple_lines () =
  ...
  let span = Loc.make_span ~start_pos:pos ~end_pos in
  let formatted = Diag.format_span span in
  Alcotest.(check bool) "contains start line" true
    (contains_pattern (Str.regexp "10") formatted);
  Alcotest.(check bool) "contains end line" true
...
let test_to_string_includes_location () =
  ...
  Alcotest.(check bool) "contains line 5" true
...
let test_to_string_includes_expected_actual () =
  ...
  Alcotest.(check bool) "contains 'found'" true
    (contains_pattern (Str.regexp_case_fold "found") str);
  Alcotest.(check bool) "contains Int" true
    (contains_pattern (Str.regexp "Int") str);
  Alcotest.(check bool) "contains String" true
...
let test_to_string_includes_related_locations () =
  ...
  let related = [{ Diag.span = span2; message = "expected type from here" }] in
  let d = Diag.type_mismatch ~span:span1 ~expected:Types.Prim.int ~actual:Types.Prim.string ~related () in
  let str = Diag.to_string d in
  Alcotest.(check bool) "contains 'note'" true
    (contains_pattern (Str.regexp_case_fold "note") str);
  Alcotest.(check bool) "contains related message" true
    (contains_pattern (Str.regexp_case_fold "expected type from here") str);
  Alcotest.(check bool) "contains related line" true
...
let test_of_unify_arity_mismatch () =
  ...
  let d = Diag.of_unify_error err in
  Alcotest.(check bool) "is error" true (Diag.is_error d);
  Alcotest.(check bool) "message contains arity" true
...
let () =
  Alcotest.run "diagnostic"
    [
      ( "creation",
        [
          Alcotest.test_case "type mismatch" `Quick test_type_mismatch_diagnostic;
          Alcotest.test_case "arity mismatch" `Quick test_arity_mismatch_diagnostic;
          Alcotest.test_case "occurs check" `Quick test_occurs_check_diagnostic;
        ] );
      ( "formatting",
        [
          Alcotest.test_case "format span same line" `Quick test_format_span_same_line;
          Alcotest.test_case "format span multi line" `Quick test_format_span_multiple_lines;
          Alcotest.test_case "includes location" `Quick test_to_string_includes_location;
          Alcotest.test_case "includes expected/actual" `Quick test_to_string_includes_expected_actual;
          Alcotest.test_case "includes related" `Quick test_to_string_includes_related_locations;
          Alcotest.test_case "compact format" `Quick test_to_string_compact;
        ] );
      ( "conversion",
        [
          Alcotest.test_case "of type mismatch" `Quick test_of_unify_type_mismatch;
          Alcotest.test_case "of occurs check" `Quick test_of_unify_occurs_check;
          Alcotest.test_case "of arity mismatch" `Quick test_of_unify_arity_mismatch;
          Alcotest.test_case "of errors list" `Quick test_of_unify_errors_list;
        ] );
      ( "integration",
        [
          Alcotest.test_case "check expr error" `Quick test_check_expr_error_diagnostic;
          Alcotest.test_case "source location" `Quick test_diagnostic_has_source_location;
