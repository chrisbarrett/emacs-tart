...
let test_same_tcon () =
  Alcotest.(check bool) "Int = Int" true
    (unify_ok Prim.int Prim.int)

let test_different_tcon () =
  Alcotest.(check bool) "Int != String" true
    (unify_fails Prim.int Prim.string)

let test_nil_nil () =
  Alcotest.(check bool) "Nil = Nil" true
    (unify_ok Prim.nil Prim.nil)

let test_bool_bool () =
  Alcotest.(check bool) "Bool = Bool" true
    (unify_ok Prim.bool Prim.bool)
...
let test_tvar_unifies_with_tcon () =
  setup ();
  let tv = fresh () in
  Alcotest.(check bool) "'a = Int" true
    (unify_ok tv Prim.int);
  (* After unification, tv should resolve to Int *)
  Alcotest.(check string) "tv resolves to Int" "Int"
    (to_string tv)
...
let test_occurs_check_simple () =
  ...
  Alcotest.(check bool) "occurs check: 'a = List 'a" true
    (unify_fails tv list_tv)
...
let test_occurs_check_nested () =
  ...
  Alcotest.(check bool) "occurs check nested" true
    (unify_fails tv nested)
...
let test_occurs_check_in_arrow () =
  ...
  Alcotest.(check bool) "occurs check in arrow" true
    (unify_fails tv fn_type)
...
let test_no_occurs_check_different_tvars () =
  ...
  Alcotest.(check bool) "'a = List 'b" true
    (unify_ok tv1 list_tv2)
...
let test_pair_unification () =
  ...
  Alcotest.(check bool) "Pair 'a 'b = Pair Int String" true
    (unify_ok p1 p2);
...
let test_arrow_with_tvars () =
  ...
  Alcotest.(check bool) "('a -> 'b) = (Int -> String)" true
    (unify_ok fn1 fn2);
  Alcotest.(check string) "tv1 = Int" "Int" (to_string tv1);
  Alcotest.(check string) "tv2 = String" "String" (to_string tv2)
...
let test_solve_empty () =
  Alcotest.(check bool) "solve empty constraints" true
...
let test_solve_single () =
  ...
  Alcotest.(check bool) "solve single constraint" true
...
let test_solve_multiple () =
  ...
  Alcotest.(check bool) "solve multiple constraints" true
...
let test_solve_chained () =
  ...
  Alcotest.(check bool) "solve chained constraints" true
    (match Unify.solve [c1; c2] with Ok () -> true | Error _ -> false);
  Alcotest.(check string) "tv1 = Int" "Int" (to_string tv1)
...
let test_solve_conflicting () =
  ...
  Alcotest.(check bool) "conflicting constraints fail" true
...
let test_tuple_different () =
  ...
  Alcotest.(check bool) "(Int, String) != (String, Int)" true
    (unify_fails t1 t2)

let test_tuple_different_length () =
  let t1 = TTuple [Prim.int] in
  let t2 = TTuple [Prim.int; Prim.string] in
  Alcotest.(check bool) "(Int) != (Int, String)" true
    (unify_fails t1 t2)
...
let test_tuple_with_tvars () =
  ...
  let t2 = TTuple [Prim.int; Prim.string] in
  Alcotest.(check bool) "('a, String) = (Int, String)" true
    (unify_ok t1 t2);
  Alcotest.(check string) "tv = Int" "Int" (to_string tv)
...
let test_identity_function () =
  ...
  Alcotest.(check bool) "('a -> 'a) = (Int -> 'b)" true
    (unify_ok fn1 fn2);
...
let test_map_function () =
  ...
  Alcotest.(check bool) "map type unifies" true
    (unify_ok map_type expected)
...
let test_nested_application () =
  ...
  Alcotest.(check bool) "List (Option 'a) = List (Option Int)" true
    (unify_ok t1 t2);
  Alcotest.(check string) "a = Int" "Int" (to_string a)
    ...
    [
      ...
      ( "occurs check",
        [
          ...
          Alcotest.test_case "different tvars ok" `Quick test_no_occurs_check_different_tvars;
        ] );
      ( "type applications",
        [
          Alcotest.test_case "List Int = List Int" `Quick test_list_int_list_int;
          Alcotest.test_case "List Int != List String" `Quick test_list_int_list_string;
          ...
          Alcotest.test_case "different constructors" `Quick test_different_constructors;
          Alcotest.test_case "pair unification" `Quick test_pair_unification;
        ] );
      ( "arrow types",
        [
          Alcotest.test_case "same arrow" `Quick test_arrow_same;
          Alcotest.test_case "different return" `Quick test_arrow_different_return;
      ...
      ( "tuples",
        [
          ...
          Alcotest.test_case "different length" `Quick test_tuple_different_length;
      ...
      ( "solve_all",
        [
          ...
          Alcotest.test_case "multiple errors" `Quick test_solve_all_multiple_errors;
        ] );
    ]
