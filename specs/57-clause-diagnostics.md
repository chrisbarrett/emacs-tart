...
Multi-clause signatures ([Spec 54](./54-multi-clause-signatures.md)) pattern-match on argument types at call sites.
...
Clauses are ordered top-to-bottom ([Spec 54](./54-multi-clause-signatures.md)). The first matching clause
...
| Severity mapping   | Maps to [Spec 51](./51-diagnostic-severity.md): error→Error, warn→Warning, note→Info |
...
### R6: Interaction with [Spec 51](./51-diagnostic-severity.md) severity flags

**Given** diagnostics emitted from clause annotations
**When** formatted for output (CLI or LSP)
**Then** map to [Spec 51](./51-diagnostic-severity.md) severity levels:
- `error` → `Severity.Error`
- `warn` → `Severity.Warning`
- `note` → `Severity.Info`

**Given** CLI flags from [Spec 51](./51-diagnostic-severity.md) (`--warn-as-error`, `--ignore-warnings`)
...
- Diagnostics on virtual clauses generated by row-typed dispatch ([Spec 56](./56-plist-type-overloading.md))
...
matches based on its parameter types ([Spec 54](./54-multi-clause-signatures.md)). If it matches AND has a
...
With [Spec 55](./55-plist-intrinsic.md) (plist intrinsic) and this spec, `plist-member` needs no special
compiler support:
...
The type-checker matches against call-site argument types ([Spec 54](./54-multi-clause-signatures.md)/[Spec 56](./56-plist-type-overloading.md)), selects
...
Virtual clauses generated during row-typed dispatch ([Spec 56](./56-plist-type-overloading.md)) do not carry
...
[Spec 56](./56-plist-type-overloading.md) for migration to multi-clause `.tart` signatures. Clause diagnostics
