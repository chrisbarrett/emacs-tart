...
let open_doc (store : t) ~(uri : string) ~(version : int) ~(text : string) : unit
    =
...
let position_to_offset (text : string) (pos : position) : int option =
  ...
  let rec find_offset line_num char_offset lines =
    ...
    | [] ->
        ...
        if line_num = pos.line && pos.character = 0 then
          Some char_offset
        else
          None
    | line :: rest ->
        if line_num = pos.line then
          ...
          if pos.character <= line_len then
            Some (char_offset + pos.character)
...
(** Apply a single content change to document text.
    Returns Error if position is out of range. *)
...
let position_of_json (json : Yojson.Safe.t) : position =
  ...
  { line = json |> member "line" |> to_int; character = json |> member "character" |> to_int }

let range_of_json (json : Yojson.Safe.t) : range =
  let open Yojson.Safe.Util in
  { start = json |> member "start" |> position_of_json; end_ = json |> member "end" |> position_of_json }
