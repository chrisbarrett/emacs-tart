(** Top-level reader API for Elisp S-expressions.
    ...
    This module provides the main entry points for parsing Elisp source
    code into S-expression ASTs. It supports error recovery, continuing
    to parse after encountering errors to produce partial results. *)
...
(** [parse_file filename] reads and parses a file into S-expressions.
    Continues parsing after errors, collecting all errors encountered. *)
...
(** [parse_one ?filename source] parses exactly one S-expression from a string.
    Returns [Error] if the input is empty, has errors, or contains multiple expressions. *)

val parse_one_exn : ?filename:string -> string -> Sexp.t
(** [parse_one_exn ?filename source] parses exactly one S-expression.
    @raise Failure if parsing fails or input doesn't contain exactly one expression. *)
