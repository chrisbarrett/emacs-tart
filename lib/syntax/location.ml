...
(** Check if a position is within a span. Uses 0-based line and column numbers
    (LSP convention). Internal positions use 1-based lines, 0-based columns. *)
let contains_position (span : span) ~(line : int) ~(col : int) : bool =
  ...
  in
  let before_end = line < end_.line || (line = end_.line && col < end_.col) in
