...
type eval_error = { message : string; span : Syntax.Location.span }
...
(** Convert a value back to an S-expression (for macro expansion). The span is
    used for the generated nodes. *)
...
val apply :
  Env.global -> Value.value -> Value.value list -> (Value.value, string) result
(** [apply global fn args] applies a function to arguments. The function must be
    a closure or builtin. *)

(** {1 Macro expansion} *)

val expand_macro_1 :
  Env.global -> Syntax.Location.span -> string -> Syntax.Sexp.t -> Syntax.Sexp.t
(** [expand_macro_1 global span name form] expands a single macro call. Returns
    the original form if [name] is not a macro. *)

val macroexpand_all : Env.global -> Syntax.Sexp.t -> Syntax.Sexp.t
(** [macroexpand_all global sexp] recursively expands all macros in a form. Does
    not expand inside [quote]. *)

(** {1 Top-level evaluation} *)

val eval_toplevel :
  Env.global -> Syntax.Sexp.t -> (Value.value, eval_error) result
(** Evaluate a top-level form, catching exceptions. *)

val eval_string : Env.global -> string -> (Value.value, eval_error) result
(** Parse and evaluate a string of code. Returns the value of the last
    expression. *)
