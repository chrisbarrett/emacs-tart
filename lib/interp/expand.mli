...
(** [expand_all global sexp] recursively expands all macros in a form. Does not
    expand inside [quote]. *)

val expand_1 : Env.global -> Syntax.Sexp.t -> expansion_result
(** [expand_1 global sexp] expands a single macro call (one step). Returns the
    unchanged form if it's not a macro call. *)

(** {1 Macro management} *)

val load_macros : Env.global -> Syntax.Sexp.t list -> unit
(** [load_macros global sexps] loads macro definitions from parsed forms. Only
    processes [defmacro] forms; other forms are ignored. Errors during macro
...
val register_macro :
  ...
  Value.env ->
  unit
(** [register_macro global name params body env] registers a macro from its
    component parts. *)
