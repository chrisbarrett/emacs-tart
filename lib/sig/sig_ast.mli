...
(** Type expressions in signature files.
    ...
    These are surface syntax types that get resolved to [Core.Types.typ]
    during loading. Unlike [Types.typ], these preserve source locations
    and don't use mutable references. *)
and sig_type =
  | STVar of string * span
    (** Type variable reference (e.g., [a]) *)
  | STCon of string * span
    (** Type constant (e.g., [int], [string], [nil]) *)
...
and sig_param =
  | SPPositional of sig_type
    (** Required positional parameter *)
  | SPOptional of sig_type
    (** Optional parameter (&optional) *)
  | SPRest of sig_type
    (** Rest parameter (&rest) *)
  | SPKey of string * sig_type
    (** Keyword parameter (&key :name type) *)
...
type decl =
  ...
  | DDefun of defun_decl
    (** [(defun name ...)] - function signature *)
  | DDefvar of defvar_decl
    (** [(defvar name type)] - variable declaration *)
  | DType of type_decl
    (** [(type name ...)] - type alias or opaque type *)
...
and defun_decl = {
  defun_name : string;
  defun_tvar_binders : tvar_binder list;  (** Type variables with optional bounds *)
...
val make_bounded_tvar_binder : name:string -> bound:sig_type -> loc:span -> tvar_binder
