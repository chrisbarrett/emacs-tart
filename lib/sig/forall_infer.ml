...
(** {1 Type Variable Collection}
    ...
    Collect type variables in left-to-right first-occurrence order. Variables
    are deduplicated (each name appears only once). *)
...
(** Add a name to the list if not already present (preserves first-occurrence
    order) *)
let add_unique name lst = if List.mem name lst then lst else lst @ [ name ]

(** Collect type variables from a sig_type. Returns variables in left-to-right
    first-occurrence order. [bound] contains already bound variables to exclude.
    [known_types] contains user-defined type names to exclude. *)
let rec collect_sig_type ~(bound : string list) ~(known_types : string list)
    (ty : sig_type) : string list =
  match ty with
  | STVar (name, _) ->
      (* Type variable - include if not primitive, not bound, not a known type *)
      if is_primitive name || List.mem name bound || List.mem name known_types
      then []
      else [ name ]
  | STCon (_, _) ->
  ...
  | STUnion (types, _) -> collect_sig_types ~bound ~known_types types
  | STTuple (types, _) -> collect_sig_types ~bound ~known_types types

(** Collect type variables from multiple types (left-to-right order) *)
and collect_sig_types ~(bound : string list) ~(known_types : string list)
    (types : sig_type list) : string list =
  List.fold_left
    (fun acc ty ->
...
and collect_param ~(bound : string list) ~(known_types : string list)
    (param : sig_param) : string list =
  match param with
...
and collect_params ~(bound : string list) ~(known_types : string list)
    (params : sig_param list) : string list =
  List.fold_left
    (fun acc param ->
...
(** Infer quantifiers for a type declaration. If type_params is empty and
    there's a body, collect type variables. Returns the declaration with params
    filled in. *)
...
(** Infer quantifiers for all declarations in a signature file. Type names
    defined in the signature are treated as known types, not type variables.
