...
(** [generalize level ty] generalizes a type at the given level.
    ...
    Finds all type variables with level > [level], and wraps the type in a
...
val generalize_if_value :
  int -> Core.Types.typ -> Syntax.Sexp.t -> Core.Type_env.scheme
(** [generalize_if_value level ty expr] generalizes only if [expr] is a
    syntactic value.
