...
val check_form : Core.Type_env.t -> Syntax.Sexp.t -> Core.Type_env.t * form_result * Unify.error list
(** [check_form env sexp] type-checks a single top-level form.

    Returns the updated environment (with new bindings from defun),
    the form result, and any type errors encountered. *)
...
val check_expr : ?env:Core.Type_env.t -> Syntax.Sexp.t -> Core.Types.typ * Unify.error list
(** [check_expr ?env sexp] type-checks a single expression.

    A convenience function for checking a single expression without
    accumulating environment changes. Returns the inferred type and
    any errors. Uses the default environment unless overridden. *)
