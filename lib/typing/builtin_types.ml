...
(** Helper to create a polymorphic type variable reference.
    This creates a named type variable for use in forall types. *)
...
let signatures : (string * Env.scheme) list = [
  ...
  ("cons", poly ["a"] (fn [tvar "a"; list_t (tvar "a")] (list_t (tvar "a"))));
  ...
  ("nth", poly ["a"] (fn [Prim.int; list_t (tvar "a")] (option_t (tvar "a"))));

  (* nthcdr : (forall (a) (-> (Int (List a)) (List a))) *)
  ("nthcdr", poly ["a"] (fn [Prim.int; list_t (tvar "a")] (list_t (tvar "a"))));

  (* append : (forall (a) (-> (&rest (List a)) (List a))) *)
  ("append", poly ["a"] (TArrow ([PRest (list_t (tvar "a"))], list_t (tvar "a"))));
  ...
  ("member", poly ["a"] (fn [tvar "a"; list_t (tvar "a")] (list_t (tvar "a"))));

  (* memq : (forall (a) (-> (a (List a)) (List a))) *)
  ("memq", poly ["a"] (fn [tvar "a"; list_t (tvar "a")] (list_t (tvar "a"))));

  (* assoc : (forall (k v) (-> (k (List (List k v))) (Option (List k v)))) *)
  (* Simplified: association list returns option of pair *)
  ("assoc", poly ["k"; "v"] (fn [tvar "k"; list_t (list_t Prim.any)] (option_t (list_t Prim.any))));

  (* assq : similar to assoc *)
  ("assq", poly ["k"; "v"] (fn [tvar "k"; list_t (list_t Prim.any)] (option_t (list_t Prim.any))));
  ...
  ("substring", mono (TArrow ([PPositional Prim.string; PPositional Prim.int; POptional Prim.int], Prim.string)));
  ...
  ("format", mono (TArrow ([PPositional Prim.string; PRest Prim.any], Prim.string)));
  ...
  ("aset", poly ["a"] (fn [vector_t (tvar "a"); Prim.int; tvar "a"] (tvar "a")));
